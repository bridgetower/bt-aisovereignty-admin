...
>(({ step, onClick }, ref) => {
  ...
  return (
    <li ref={ref} className="flex flex-col cursor-pointer my-4 relative w-full">
      <div className="flex items-center" onClick={onClick}>
        ...
        <div
          className={`ml-2 flex items-center text-sm ${
            completed ? 'text-[#1890FF]' : 'text-gray-500'
          }`}
        >
      ...
      {!isLast && (
        <span
          className={`absolute top-10 left-[45px] w-[1.5px] opacity-70 ${
            completed ? 'bg-[#1890FF]' : 'border border-dotted'
          }`}
            ...
            {(step.stepdetails || []).map((d: any, i: number) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="underline text-xs w-[350px] truncate">
                    {data?.hash}
                  </div>
                  <CheckCircle2
                    className="text-green-500"
